...
term, the diffusion coefficient being $\Delta x|\lambda_p|$. Also note
that each Riemann variable has, in general, a \emph{different}
...
in which we split instead the jump in conserved variables.
...
\begin{align}
  \mathcal{W}^{1,2} &= \frac{1}{2} \left( Q_R - Q_L \right) \mp \frac{1}{2 |\lambda |} \left[ F(Q_R) - F(Q_L) \right]
\end{align}
